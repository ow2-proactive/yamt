...
import org.ow2.proactive.smart.watch.fixtures.RuleFixtures;
import org.ow2.proactive.smart.watch.model.Node;
...
import org.ow2.proactive.smart.watch.rules.loader.RulesLoader;
...
//				.thenReturn(new NodeInformation("proactiveId1", "OPENSTACK"));
	...
	public void testFullLoadWithInit() {
		...
		assertThat(rules.get("rule1").getRule(), is(RuleFixtures.getSimpleRule("rule1")));
	...
	public void testFullLoadWithRefreshData() {
		...
		Map<String, RuleEPRuntimeTuple> rules = smartWatchCache.getRules();
		assertThat(rules.size(), is(1));
		assertThat(rules.get("rule3").getRule(), is(RuleFixtures.getSimpleRule("rule3")));
	}
	...
	public void testFullLoadWithRefreshDataMultipleRules() {
		...
		assertThat(rules.get("rule4").getRule(), is(RuleFixtures.getSimpleRule("rule4")));
		assertThat(rules.get("rule5").getRule(), is(RuleFixtures.getSimpleRule("rule5")));
		assertThat(rules.get("rule6").getRule(), is(RuleFixtures.getSimpleRule("rule6")));
	}
